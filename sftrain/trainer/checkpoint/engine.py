...
from sftrain.python.common import env_utils
from sftrain.python.common.constants import CheckpointConstant
from sftrain.python.common.log import default_logger as logger
from sftrain.python.common.multi_process import SharedLock, SharedQueue
from sftrain.python.common.singleton import Singleton
from sftrain.python.common.storage import CheckpointStorage
from sftrain.python.elastic_agent.torch.ckpt_saver import (
    SFTRAIN_CKPT_CONFIG_KEY,
...
from .replica import CkptReplicaManger
...
def start_saver_process():
    ...
    process is not launched by `sfrun`. This process will
    exit and cannot save the checkpoint after the training process exit.
    It is better to use `sfrun` to start the training process.
    `sfrun` can save checkpoint once the training process fails
    ...
    # if the training process is not launched by sfrun.
    if role_name != "sftrain-trainer" and local_rank == 0:
...
class CheckpointEngine(metaclass=ABCMeta):
    ...
    def save_state_dict_to_memory(self, state_dict, conf: CheckpointConfig):
        ...
        if not all_rank_ready or not state_dict:
            ...
            return False
        state_dict[SFTRAIN_CKPT_CONFIG_KEY] = conf
    ...
    def get_state_dict_from_memory(self):
        ...
        if passed and config.step > 0:
            ...
            state_dict.pop(SFTRAIN_CKPT_CONFIG_KEY, None)
